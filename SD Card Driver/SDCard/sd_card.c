 ...
 //   if(iFResult != FR_OK) {
  ...
  //  }
    ...
    FIL fsrc;                /* File objects */
      ...
      if (! f_size(&fsrc)) {
          f_printf(&fsrc, "Battery Pack Status\t\tSeries cell's Voltages\t\t\t\t\t\tSeries cell's SOC\t\t\t\t\t\tTemperature Sensors data\t\t\t\t\tBalancing Cell ID\n");
//          f_printf(&fsrc,"Time\tSOC\tVoltage\tCurrent\tTemp.\t");
          f_printf(&fsrc,"SOC\tVoltage\tCurrent\tTemp.\t");
          for(i = 0; i < noSCell; i++){
              f_printf(&fsrc,"%d\t",i+1);  // printf("Cell_Voltage:\t");
          }

          for(i = 0; i < noSCell; i++){
              f_printf(&fsrc,"%d\t",i+1);  // printf("Cell_SOC:\t");
          }

          for(i = 0; i < noTemp; i++){
              f_printf(&fsrc,"%d\t",i+1);  // printf("Temp_Sensors:\t");
          }

          for(i = 0; i < noSCell; i++){
              f_printf(&fsrc,"%d\t",i+1);   // printf("Cell no\t");
          }
          res = f_printf(&fsrc,"\n");
       if (res <0) {
           /* Error. Cannot write header */
...
//      f_printf(&fsrc,"%d\t%d\t%d\t%d\t%d\t",(int)timer,Pack_Z,Pack_V,Pack_I,Pack_T);

      f_printf(&fsrc,"%d\t%d\t%d\t%d\t",Pack_Z,Pack_V,Pack_I,Pack_T);
//    printf("\t");
      for(i = 0; i < noSCell; i++){
          f_printf(&fsrc,"%d\t",Cell_V[i]);// c is structure voltage data
      }
//      f_printf(&fsrc,"\n");
      for(i = 0; i < noSCell; i++){
         f_printf(&fsrc,"%d\t",Cell_Z[i]);
      }
////    printf("\t\t");
      for(i = 0; i < noTemp; i++){
         f_printf(&fsrc,"%d\t",Cell_T[i]);
      }
      res = f_printf(&fsrc,"\n ");
//      res = f_printf(&fsrc, "%d;%d;%d;%d\n", a*1000, 2, 3, Pack_V);
      if (res < FR_OK) {
